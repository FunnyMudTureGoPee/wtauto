...
from presskey import hold, wtpress, key_down, key_up, press, wtpressfast
...
def randomAttack():  # 随机拉提前量 组合动作花费1s
    wtpressfast(k=key.key_Numpad0)
    r = random.randint(1, 10)
    print('r={}'.format(r))
    if r <= 3:
        wtpressfast(k=key.key_Left)
    elif r >= 9:
        wtpressfast(k=key.key_Right)
    elif r == 5:
        wtpressfast(k=key.key_Up)
    elif r == 4:
        wtpressfast(k=key.key_Down)
    wtpressfast(k=key.key_RightControl)


def attackTarget():  # 攻击目标
    global flag
    i = 0
    while True:
        if flag:
            return
        i=i+1
        key_down(k=key.key_X)
        time.sleep(0.3)
        key_up(k=key.key_X)
        log('选择目标')
        wtpressfast(k=key.key_P)  # 镜头锁定
        wtpressfast(k=key.key_RightControl)  # 首轮试射
        randomAttack()  # 1.5s
        randomAttack()  # 2.5s
        log('射击完毕')
        print('第{}轮射击'.format(i))
...
def main_run():
    ...
    prepare()
    Attack = executor.submit(attackTarget)
